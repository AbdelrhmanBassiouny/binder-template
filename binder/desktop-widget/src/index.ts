...
const DEFAULT_APP: AppId = 'desktop';
...
/**
 * A panel that embeds the remote desktop and VSCode, and switches between
 * them without reloading either one.
 */
class DesktopContent extends Widget {
   ...
   *
   * Iframes are kept alive and only hidden, so switching back to an app keeps
   * the desktop session and the editor state intact.
   */
...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (
    ...
    app.commands.addCommand(SWITCH_COMMAND_ID, {
      label: 'Switch Between Desktop and VSCode',
      execute: async () => {
