...
update_cognitive_architecture() {
    ...
    (cd "${repo_dir}" && python3 scripts/regenerate_all_orm.py) >/tmp/cognitive-architecture-orm.log 2>&1 || \
        echo "ORM interface generation failed; see /tmp/cognitive-architecture-orm.log" >&2
}
